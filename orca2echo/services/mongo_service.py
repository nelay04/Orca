...
def update_fields_by_email(collection_name:str, email: str, updates: dict):  # Update user document(s) by email with multiple fields.
...
def update_fields_by_user_name(collection_name:str, user_name: str, updates: dict):  # Update user document(s) by email with multiple fields.
    collection = db[collection_name]
    if "email" in updates:
        raise ValueError("Email field cannot be modified.")
...
def get_user_by_id(collection_name:str, user_id:str):  # Retrieve a user document by id.
    try:
        collection = db[collection_name]
